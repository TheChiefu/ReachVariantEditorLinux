...
void* ReachStringTable::_make_buffer(cobb::ibitreader& stream) const noexcept {
   ...
   if (is_compressed) {
      if (size < sizeof(uint32_t)) {
         ...
         error_report.extra[0]      = Z_DATA_ERROR;
         return nullptr;
      }
      //
      ...
      //
      uint32_t serialized_uncompressed_size = 0;
      std::memcpy(&serialized_uncompressed_size, buffer, sizeof(serialized_uncompressed_size));
      if (serialized_uncompressed_size != uncompressed_size)
         if (cobb::byteswap(serialized_uncompressed_size) != uncompressed_size)
            printf("WARNING: String table sizes don't match: Bungie 0x%08X versus zlib 0x%08X.", uncompressed_size, serialized_uncompressed_size);
      ...
      //
      if (final) {
         Bytef* input = (Bytef*)((std::uintptr_t)buffer + sizeof(serialized_uncompressed_size));
         uLongf output_size = static_cast<uLongf>(uncompressed_size);
         int resultCode = uncompress(
            (Bytef*)final,
            &output_size,
            input,
            static_cast<uLongf>(size - sizeof(serialized_uncompressed_size))
         );
         if (Z_OK != resultCode) {
            free(final);
            final = nullptr;
